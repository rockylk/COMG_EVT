...
from sklearn.ensemble import IsolationForest  # [IM-ZDD] 核心隔离机制
import warnings
...
LOG_FILE = os.path.join(RESULT_DIR, f"IM_ZDD_Baseline_Log_{int(time.time())}.txt")
...
class Logger:
    def __init__(self, filepath):
        ...
        self.log(f"=== IM-ZDD Baseline (DNN + Isolation Forest ZDD) Log ===")
...
# ==========================================
# 3. 骨干架构 (DNN Feature Extractor)
# ==========================================
class IM_ZDD_Backbone(nn.Module):
    def __init__(self, num_classes, in_channels=5):
        super(IM_ZDD_Backbone, self).__init__()
...
class IM_ZDD_Engine:
    ...
    def fit(self, features, logits, labels, known_classes, logger):
        """为每个已知类独立拟合一个隔离森林 (Per-class Isolation Forest)"""
        ...
        for c in known_classes:
            ...
            # 训练隔离森林 (contamination 设置为 5% 作为预期离群值比例)
            clf = IsolationForest(n_estimators=100, contamination=0.05, random_state=42)
            clf.fit(class_features)
            self.iso_models[c] = clf
            
        logger.log(f"  [IM-ZDD] Calibrated Isolation Forest ZDD boundaries for {len(self.iso_models)} classes.")

    def predict_score(self, feature, logit):
        """在线阶段：隔离森林打分并进行 Sigmoid 概率化映射"""
        pred_cls = np.argmax(logit)
        clf = self.iso_models.get(pred_cls)
        
        if clf is None:
            return pred_cls, 0.0 
            
        # decision_function 返回值：>0 为正常 (inlier)，<0 为异常 (outlier)
        score = clf.decision_function([feature])[0]
        
        # 为了与 EVT 计算 OSCR 曲线兼容，我们将 (-inf, +inf) 映射到 (0, 1) 的置信度
        # 乘以 10 是为了拉开分数梯度。当 score = 0 时，置信度刚好 = 0.5 (对应 tau)
        confidence_inlier = 1.0 / (1.0 + np.exp(-score * 10))
...
# 5. 训练与测试流程
# ==========================================
def train_im_zdd_baseline(known_data_list, unknown_data_list, num_known_classes, idx_to_class, logger):
    ...
    model = IM_ZDD_Backbone(num_classes=num_known_classes).to(DEVICE)
    ...
    # --- Stage 2: Extract Deep Features & Fit Isolation Forests ---
    logger.log(f"\n>>> [Stage 2] Offline Isolation Forest Fitting on Deep Features...")
    model.eval()
    all_train_features = []
    all_train_logits = []
    all_train_labels = []
    ...
    zdd_engine = IM_ZDD_Engine()
    zdd_engine.fit(all_train_features, all_train_logits, all_train_labels, list(range(num_known_classes)), logger)

    # --- Stage 3: Online Detection & Table II Metrics ---
    logger.log("\n>>> [Stage 3] Calculating Table II Metrics (IM-ZDD AUROC, AUOSCR, CCR, FPR)...")
    ...
    with torch.no_grad():
        for batch_x, batch_y in test_loader:
            ...
            for i in range(len(features_np)):
                pred_cls, conf = zdd_engine.predict_score(features_np[i], logits_np[i])
    ...
    # 3. Metrics @ τ = 0.5 (Sigmoid(0) = 0.5，恰好对应 Isolation Forest 原生的零界点)
    tau = 0.5
    ...
    # 输出表格结果
    logger.log("\n" + "="*50)
    logger.log(" 🏆 Table II: Open-Set Metrics (Baseline: IM-ZDD / Isolation Forest)")
    logger.log("="*50)
    logger.log(f"  * AUROC  : {auroc:.2f}%  (vs. Unknowns)")
    logger.log(f"  * AUOSCR : {auoscr:.2f}%  (Area under Curve)")
    logger.log(f"  * CCR    : {ccr_tau:.2f}%  (@ Native ZDD Boundary τ={tau})")
...
if __name__ == "__main__":
    ...
    if os.path.exists(DATASET_PATH):
        ...
        if len(k_list) > 0:
            train_im_zdd_baseline(k_list, u_list, n_k, idx_cls, logger)
    else:
